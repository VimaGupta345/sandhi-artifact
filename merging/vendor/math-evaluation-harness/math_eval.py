...
def setup(args):
    ...
    if args.use_vllm:
        llm = LLM(model=args.model_name_or_path, tensor_parallel_size=len(available_gpus), trust_remote_code=True, gpu_memory_utilization=0.8)  # 0.8: coexist with the caller's GPU-resident model (0.9 refuses to boot next to a 15GB parent on 144GB)
        tokenizer = None
    else:
